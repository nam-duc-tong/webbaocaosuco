...
// Hàm gửi email thông báo cập nhật trạng thái cho người dùng
function sendStatusUpdateEmail($to_email, $to_name, $report_id, $status, $admin_notes)
{
    $mail = new PHPMailer\PHPMailer\PHPMailer(true);

    try {
        // Cấu hình SMTP (dùng chung cấu hình từ mail_config)
        $mail->isSMTP();
        $mail->Host = SMTP_HOST;
        $mail->SMTPAuth = true;
        ...
        $status_display = $status_text[$status] ?? $status;

        // Link xem chi tiết
        $protocol = isset($_SERVER['HTTPS']) ? 'https://' : 'http://';
        $host = $_SERVER['HTTP_HOST'];
        $detail_url = $protocol . $host . '/track_report.php?id=' . $report_id;

        // Nội dung email
        $mail->Subject = '📋 CẬP NHẬT TRẠNG THÁI BÁO CÁO #' . str_pad($report_id, 5, '0', STR_PAD_LEFT);
        ...
        <head>
            ...
            <style>
                ...
                .btn { background: #28a745; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; margin-top: 10px; }
            </style>
        </head>
        <body>
            <div class="container">
                <div class="header">
                    ...
                    <p>Mã báo cáo: #' . str_pad($report_id, 5, '0', STR_PAD_LEFT) . '</p>
                </div>
                
                <div class="content">
                    <p>Xin chào <strong>' . htmlspecialchars($to_name) . '</strong>,</p>
                    <p>Báo cáo sự cố của bạn đã được cập nhật trạng thái mới:</p>
                    ...
                    <div class="info-row">
                        <span class="label">📝 Ghi chú từ admin:</span>
                        <div style="margin-top: 5px; padding: 10px; background: #e9ecef; border-radius: 5px;">
                            ' . nl2br(htmlspecialchars($admin_notes ?: 'Không có ghi chú thêm')) . '
                        </div>
                    </div>
                    
                    <div style="text-align: center; margin-top: 20px;">
                        <a href="' . $detail_url . '" class="btn">🔍 XEM CHI TIẾT BÁO CÁO</a>
                    </div>
                    
                    <div style="margin-top: 20px; background: #e7f3ff; padding: 10px; border-radius: 5px;">
                        <p>📌 <strong>Hướng dẫn:</strong> Nhấn vào nút trên để xem chi tiết báo cáo của bạn.</p>
        ...
        $mail->Body = $body;
        $mail->AltBody = "CẬP NHẬT TRẠNG THÁI BÁO CÁO #" . str_pad($report_id, 5, '0', STR_PAD_LEFT) . "\n\nTrạng thái: " . $status_display . "\nGhi chú: " . ($admin_notes ?: 'Không có') . "\n\nXem chi tiết: " . $detail_url;

        $mail->send();
        return true;
    } catch (Exception $e) {
        error_log("Email thông báo cập nhật không gửi được: " . $mail->ErrorInfo);
        return false;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Chỉ gửi email nếu trạng thái thay đổi và có email người dùng
    if ($old_status != $status && !empty($user_email)) {
        $email_sent = sendStatusUpdateEmail($user_email, $user_name, $id, $status, $admin_notes);
        if ($email_sent) {
            $_SESSION['email_notify'] = "Đã gửi email thông báo đến người dùng!";
        } else {
            $_SESSION['email_notify'] = "Không thể gửi email thông báo!";
        }
    }
...
<body>
    ...
    <div class="main-content">
        <div class="navbar-top">
            ...
            <div>
                <span class="me-3"><i class="fas fa-user"></i> <?php echo htmlspecialchars($_SESSION['nguoibaocao'] ?? 'Admin'); ?></span>
        ...
        <?php if (isset($_GET['updated'])): ?>
            <div class="alert alert-success alert-dismissible fade show" role="alert">
                ...
                <?php if (isset($_SESSION['email_notify'])): ?>
                    <br><i class="fas fa-envelope"></i> <?php echo $_SESSION['email_notify']; ?>
                    <?php unset($_SESSION['email_notify']); ?>
                <?php endif; ?>
        ...
        <?php if (isset($_GET['deleted'])): ?>
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                <i class="fas fa-trash"></i> Đã xóa báo cáo thành công!
                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
            </div>
        <?php endif; ?>
        <?php if (isset($_GET['updated'])): ?>
        ...
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <div class="info-value">
                    ...
                    // Lấy dữ liệu phân loại từ database
                    $phan_loai_data = [];

                    // Ưu tiên lấy từ cột JSON trước
                    if (!empty($report['phan_loai_su_co_json'])) {
                        if (is_string($report['phan_loai_su_co_json'])) {
                            $phan_loai_data = json_decode($report['phan_loai_su_co_json'], true);
                        } else {
                            $phan_loai_data = $report['phan_loai_su_co_json'];
                        }
                    }

                    // Nếu JSON rỗng, lấy từ cột text
                    if (empty($phan_loai_data) && !empty($report['phan_loai_su_co'])) {
                        // Xử lý chuỗi text, tránh bị tách từng từ
                        $phan_loai_data = explode(', ', $report['phan_loai_su_co']);
                    }

                    // Đảm bảo là mảng
                    if (!is_array($phan_loai_data)) {
                        $phan_loai_data = [];
                    }

                    // Lọc chỉ lấy các giá trị hợp lệ (có trong $valid_categories)
                    $phan_loai_data = array_filter($phan_loai_data, function ($item) use ($valid_categories) {
                        return in_array(trim($item), $valid_categories);
                    });

                    if (!empty($phan_loai_data)):
                    ?>
                        <div class="row">
                            <?php foreach ($phan_loai_data as $item): ?>
                                <div class="col-md-6 mb-2">
                                    <span class="badge bg-primary" style="padding: 6px 12px; font-size: 13px;">
                                        <i class="fas fa-check-circle"></i> <?php echo htmlspecialchars(trim($item)); ?>
                                    </span>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    <?php else: ?>
                        <span class="text-muted">Không có phân loại nào</span>
                    <?php endif; ?>
                </div>
            </div>
        </div>
